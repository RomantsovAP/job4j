...
public class IteratorOfIteratorsTest {
    ...
    @Test
    public void testsThatNextMethodDoesntDependsOnPriorHasNextInvocatio() {
        assertThat(it.next(), is(1));
        assertThat(it.next(), is(2));
        assertThat(it.next(), is(3));
        assertThat(it.next(), is(4));
    ...
    @Test(expected = NoSuchElementException.class)
    public void whenConvertsEmptyIteratorItReturnsCorrectIterator() {
        Iterator<Iterator<Integer>> iteratorIterator = new ArrayList<Iterator<Integer>>().iterator();
        IteratorOfIterators converter = new IteratorOfIterators();
        it = converter.convert(iteratorIterator);
        assertThat(it.hasNext(), is(false));
        it.next();
    }
